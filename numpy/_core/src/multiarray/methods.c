...
/*
 * ndarray.filled(fill_value=0): a plain copy in which every hidden element is
 * replaced by `fill_value` (assigned like `arr[i] = fill_value`, so it must be
 * representable in the dtype). Without `fill_value` the zero of the dtype is
 * used, as `np.zeros` builds it (0, False, '', b'', 1970-01-01, None-like for
 * object). The result never carries a mask and never shares memory with
 * `self`; an unmasked array is simply copied.
 */
static PyObject *
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    /* The fill element; validated even when there is nothing to replace. */
    PyArray_Descr *descr = PyArray_DESCR(self);
    Py_INCREF(descr);
    PyArrayObject *src;
    if (fill_value == NULL) {
        src = (PyArrayObject *)PyArray_Zeros(0, NULL, descr, 0);  /* steals */
        if (src == NULL) {
            return NULL;
        }
    }
    else {
        src = (PyArrayObject *)PyArray_NewFromDescr(
                &PyArray_Type, descr, 0, NULL, NULL, NULL, 0, NULL);
        if (src == NULL) {
            return NULL;
        }
        if (PyArray_Pack(PyArray_DESCR(src), PyArray_DATA(src),
                         fill_value) < 0) {
            Py_DECREF(src);
            return NULL;
        }
